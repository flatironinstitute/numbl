...
describe("flame-blas-lapack wasm smoke", () => {
  const repoRoot = process.cwd();
  const wasmPath = join(
    repoRoot,
    "public",
    "wasm-kernels",
    "flame-blas-lapack.wasm"
  );
  ...
  maybeIt("instantiates and executes small linalg kernels", async () => {
    ...
    const rectA = new Float64Array([1, 0, 1, 0, 1, 1]);
    const rectB = new Float64Array([1, 2, 3]);
    const rectExpected = Array.from(tsBridge.linsolve!(rectA, 3, 2, rectB, 1));
    const rectAPtr = writeF64(Array.from(rectA));
    const rectBPtr = writeF64(Array.from(rectB));
    const rectOutPtr = malloc(align8(2 * Float64Array.BYTES_PER_ELEMENT));
    expect(linsolve(rectAPtr, 3, 2, rectBPtr, 1, rectOutPtr)).toBe(0);
    closeTo(readF64(rectOutPtr, 2), rectExpected);
