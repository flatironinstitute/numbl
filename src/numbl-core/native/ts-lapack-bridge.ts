...
function qr(
  ...
  const R = allocFloat64Array(rRows * n);
  for (let j = 0; j < n; j++) {
  ...
  if (wantQ) {
    ...
    const qBuf = allocFloat64Array(m * qCols);
...
function linsolve(
  ...
  if (m === n) {
    ...
    const a = allocFloat64Array(A);
    const b = allocFloat64Array(B); // n × nrhs column-major
    const ipiv = new Int32Array(n);
    ...
    const x = allocFloat64Array(n * nrhs);
    for (let c = 0; c < nrhs; c++) {
      for (let i = n - 1; i >= 0; i--) {
        let val = b[i + c * m];
        for (let k = i + 1; k < n; k++) {
    ...
    const tauT = allocFloat64Array(kk);
    ...
    const Y = allocFloat64Array(m * nrhs);
    for (let c = 0; c < nrhs; c++) {
      for (let i = 0; i < m; i++) {
        let val = B[i + c * m];
        for (let k = 0; k < i; k++) {
...
function eig(
  ...
  const a = allocFloat64Array(data); // dgeev overwrites A
  const wr = allocFloat64Array(n);
  const wi = allocFloat64Array(n);
  const ldvl = computeVL ? n : 1;
  const ldvr = computeVR ? n : 1;
  const vl = allocFloat64Array(ldvl * (computeVL ? n : 0));
  const vr = allocFloat64Array(ldvr * (computeVR ? n : 0));

  const jobvl = computeVL ? 1 : 0; // 0='N', 1='V'
  const jobvr = computeVR ? 1 : 0;

  // Workspace query
  const workQuery = allocFloat64Array(1);
  dgeev(
    jobvl,
    jobvr,
    n,
...
function linsolveComplex(
  ...
  if (m === n) {
    ...
    const aRe = allocFloat64Array(ARe);
    const aIm = allocFloat64Array(AIm);
    ...
    const aRe = allocFloat64Array(ARe);
    const aIm = allocFloat64Array(AIm);
    const tauRe = allocFloat64Array(kk);
    const tauIm = allocFloat64Array(kk);
    ...
    const xRe = allocFloat64Array(n * nrhs);
    const xIm = allocFloat64Array(n * nrhs);
    for (let c = 0; c < nrhs; c++) {
      for (let i = n - 1; i >= 0; i--) {
        let valRe = bRe[i + c * m];
        let valIm = bIm[i + c * m];
    ...
    const ahRe = allocFloat64Array(n * m);
    const ahIm = allocFloat64Array(n * m);
    for (let i = 0; i < m; i++) {
      for (let j = 0; j < n; j++) {
    ...
    const yRe = allocFloat64Array(m * nrhs);
    const yIm = allocFloat64Array(m * nrhs);
    for (let c = 0; c < nrhs; c++) {
      for (let i = 0; i < m; i++) {
        let valRe = BRe[i + c * m];
        let valIm = BIm[i + c * m];
...
function svd(
  ...
  const u = allocFloat64Array(ldu * uCols);
  const vt = allocFloat64Array(ldvt * n);
  ...
  const v = allocFloat64Array(n * vCols);
...
function cholComplex(
  ...
  const re = allocFloat64Array(dataRe);
  const im = allocFloat64Array(dataIm);
  let info = 0;
...
function gmresComplex(
  ...
  if (M1Re || M2Re) {
    const m1luRe = M1Re ? allocFloat64Array(M1Re) : null;
    const m1luIm = M1Im ? allocFloat64Array(M1Im) : null;
    ...
    const m2luRe = M2Re ? allocFloat64Array(M2Re) : null;
    const m2luIm = M2Im ? allocFloat64Array(M2Im) : null;
