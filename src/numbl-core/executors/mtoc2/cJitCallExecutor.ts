/**
 * mtoc2-cjit-call — C-backed JIT executor for user-function calls.
 *
 ...
 *     (`mtoc2TypeToCDecl` returns null).
 *   - mtoc2 throws `UnsupportedConstruct` / `Mtoc2TypeError` at lowering.
 */
...
export const mtoc2CJitCallExecutor: Executor<
  Mtoc2CJitCallData,
  CompiledArtifact | null
> = {
  name: "mtoc2-cjit-call",

  propose(
    lowered: LoweredStmt,
    ctx: DispatchContext
  ): Proposal<Mtoc2CJitCallData> | null {
  ...
  compile(d, ctx: DispatchContext): CompiledArtifact | null {
    ...
    interp.onJitCompile?.(
      `mtoc2-cjit-call:${cName}(${typeDesc}) -> nargout=${d.nargout}`,
      source
    );
  ...
  run(compiled, d, ctx): RunResult {
    if (compiled === null) {
      return { bail: { message: "mtoc2-cjit-call: codegen declined" } };
    }
    const interp = ctx.interp as Interpreter;
    const bridge = interp.nativeBridge;
    if (!bridge || !bridge.koffi) {
      return { bail: { message: "mtoc2-cjit-call: nativeBridge gone" } };
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    try {
      ...
      if (inputs === null) {
        return {
          bail: { message: "mtoc2-cjit-call: input marshal declined" },
        };
      }
      ...
      return {
        bail: {
          message: `mtoc2-cjit-call: runtime error: ${e instanceof Error ? e.message : String(e)}`,
        },
      };
    }
  },
