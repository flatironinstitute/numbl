 ...
 *   - the LAST element of a MULTI-element range (count > 1, i ==
 *     count - 1) snaps to exactly `end` when the computed value is
 *     within `|step| * 1e-10` of `end`. Without the snap, ranges like
 *     `0.1:0.1:0.3` produce a last element of `0.3000000000000000444...`,
 *     so `v(end) == 0.3` evaluates to false even though the user wrote
 *     `:0.3`. A single-element range (count == 1) is just `start` and
 *     must NOT snap, matching makeRangeTensor's `n > 1` guard.
 *
...
static double mtoc2_range_value(
  ...
  if (count > 1 && i == count - 1 && fabs(v - end) < fabs(step) * 1e-10)
    return end;
  return v;
}
