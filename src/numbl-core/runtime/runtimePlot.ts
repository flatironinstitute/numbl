...
export function fplotCall(
  ...
  const nPoints = 200;
  const t = allocFloat64Array(nPoints);
  for (let i = 0; i < nPoints; i++) {
    t[i] = interval[0] + ((interval[1] - interval[0]) * i) / (nPoints - 1);
  }

  // Evaluate function(s)
  const xArr = allocFloat64Array(nPoints);
  const yArr = allocFloat64Array(nPoints);
  ...
  const xTensor = RTV.tensor(allocFloat64Array(xArr), [1, nPoints]);
  const yTensor = RTV.tensor(allocFloat64Array(yArr), [1, nPoints]);
...
export function fplot3Call(
  ...
  // Generate sample points
  const nPoints = 200;
  const tArr = allocFloat64Array(nPoints);
  for (let i = 0; i < nPoints; i++) {
  ...
  const xArr = allocFloat64Array(nPoints);
  const yArr = allocFloat64Array(nPoints);
  const zArr = allocFloat64Array(nPoints);
  ...
  const xTensor = RTV.tensor(allocFloat64Array(xArr), [1, nPoints]);
  const yTensor = RTV.tensor(allocFloat64Array(yArr), [1, nPoints]);
  const zTensor = RTV.tensor(allocFloat64Array(zArr), [1, nPoints]);
  const synthArgs: RuntimeValue[] = [
    xTensor,
    yTensor,
    zTensor,
...
export function quiverCall(
  plotInstructions: PlotInstruction[],
  args: RuntimeValue[]
): void {
  const traces = parseQuiverArgs(args);
  if (traces.length > 0) {
...
function parseStreamlineArgs(args: RuntimeValue[]):
  ...
  if (nPositional === 4) {
    ...
    const xData = allocFloat64Array(rows * cols);
    const yData = allocFloat64Array(rows * cols);
...
export function stream2Call(args: RuntimeValue[]): RuntimeValue {
  ...
  const cellData: RuntimeValue[] = streamlines.map(s => {
    const n = s.x.length;
    const data = allocFloat64Array(n * 2);
    for (let i = 0; i < n; i++) {
