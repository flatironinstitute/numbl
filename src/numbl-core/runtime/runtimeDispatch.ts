...
function packResults(results: RuntimeValue[], shape: number[]): RuntimeValue {
  ...
  if (allScalar) {
    const data = zeroedFloatX(results.length);
...
function coerceToTensor(
  ...
  if (isRuntimeNumber(v)) {
    return RTV.tensor(copyFloatX([v]), [1, 1]);
  }
...
function arrayfunCellfunImpl(
  ...
  if (isRuntimeTensor(arrArg) && extraInputs.length === 0 && nargout <= 1) {
    ...
    const resultData = zeroedFloatX(arrArg.data.length);
    let allLogical = true;
    for (let i = 0; i < arrArg.data.length; i++) {
      const r = callFn(collectArgs(i));
      if (typeof r === "number") {
  ...
  if (args.length >= 2) {
    ...
    if (isRuntimeChar(fnArg) || isRuntimeString(fnArg)) {
      ...
      if (fnName === "isclass") {
        ...
        const data = zeroedFloatX(cellArg.data.length);
...
export function structfunImpl(
  ...
  if (uniformOutput) {
    ...
    const data = zeroedFloatX(fieldNames.length);
  ...
  for (let i = 0; i < totalElems; i++) {
    ...
    if (typeof r === "number") {
      ...
      if (isRuntimeComplexNumber(rv)) {
        resultData[i] = rv.re;
        if (!resultImag) resultImag = zeroedFloatX(totalElems);
        resultImag[i] = rv.im;
      } else {
        resultData[i] = toNumber(rv);
      }
