...
export function ensureRuntimeValue(v: unknown): RuntimeValue {
  if (v === undefined) return RTV.tensor(zeroedFloatX(0), [0]);
  if (!isRuntimeValue(v)) {
...
export function elementWiseLogicalOp(
  ...
  const aData = aIsT ? a.data : copyFloatX([toNumber(a)]);
  const bData = bIsT ? b.data : copyFloatX([toNumber(b)]);
  const aShape = aIsT ? a.shape : [1, 1];
  const bShape = bIsT ? b.shape : [1, 1];
  if (aData.length === bData.length) {
    const result = zeroedFloatX(aData.length);
  ...
  if (aData.length === 1) {
    const result = zeroedFloatX(bData.length);
