...
export function indexStore(
    ...
    if (isRuntimeStruct(rhsMv)) {
      ...
      for (let ei = 0; ei < existingElements.length; ei++) {
        ...
        for (const f of allFieldNames) {
          if (!el.fields.has(f)) {
            el.fields.set(f, RTV.tensor(zeroedFloatX(0), [0, 0]));
          }
        }
      }
      while (existingElements.length <= k) {
        existingElements.push(
          RTV.struct(
            new Map(
              allFieldNames.map(f => [f, RTV.tensor(zeroedFloatX(0), [0, 0])])
            )
          )
        );
      }
  ...
  if (!isRuntimeTensor(mv)) {
    if (isRuntimeNumber(mv)) {
      mv = RTV.tensor(copyFloatX([mv]), [1, 1]);
      wasScalar = true;
    } else if (isRuntimeLogical(mv)) {
      const t = RTV.tensor(copyFloatX([mv ? 1 : 0]), [1, 1]);
      t._isLogical = true;
      mv = t;
      wasScalar = true;
    } else if (isRuntimeComplexNumber(mv)) {
      mv = RTV.tensor(copyFloatX([mv.re]), [1, 1], copyFloatX([mv.im]));
      wasScalar = true;
    } else {
      mv = RTV.tensor(zeroedFloatX(0), [0, 0]);
    }
  }
  const idxMvals = resolveIndices(indices, endResolver(mv, indices.length));
  const rhsMv = ensureRuntimeValue(rhs);
