...
function maybePi(piList: number[]): Float64Array | undefined {
  for (let i = 0; i < piList.length; i++) {
    if (piList[i] !== 0) return allocFloat64Array(piList);
  }
  return undefined;
}
...
function sparseScaleComplex(
  ...
  const pi = allocFloat64Array(nnz);
  for (let i = 0; i < nnz; i++) {
    const aRe = S.pr[i];
...
function sparseScale(
  ...
  const pr = allocFloat64Array(S.pr.length);
  for (let i = 0; i < pr.length; i++) pr[i] = S.pr[i] * scalar;
  const pi = S.pi ? allocFloat64Array(S.pi.length) : undefined;
  if (pi && S.pi) {
...
export function sparseTranspose(S: RuntimeSparseMatrix): RuntimeSparseMatrix {
  ...
  const tPr = allocFloat64Array(nnz);
  const tPi = S.pi ? allocFloat64Array(nnz) : undefined;
...
export function sparseConjugateTranspose(
  ...
  const tJc = new Int32Array(S.m + 1);
  const tPr = allocFloat64Array(nnz);
  const tPi = allocFloat64Array(nnz);
...
function sparseMatMul(
  ...
  const accRe = allocFloat64Array(m);
  const accIm = hasImag ? allocFloat64Array(m) : null;
...
export function mAddSparse(a: RuntimeValue, b: RuntimeValue): RuntimeValue {
  ...
  if (isRuntimeSparseMatrix(a)) {
    ...
    if (cs) {
      ...
      const data = allocFloat64Array(dense.data.length);
      for (let i = 0; i < data.length; i++) data[i] = dense.data[i] + cs.re;
      const imag =
        cs.im !== 0 || dense.imag
          ? (() => {
              const r = allocFloat64Array(data.length);
...
export function mMulSparse(a: RuntimeValue, b: RuntimeValue): RuntimeValue {
  ...
  if (isRuntimeSparseMatrix(a) && isRuntimeTensor(b)) {
    ...
    const result = allocFloat64Array(m * bCols);
    const resultImag = hasImag ? allocFloat64Array(m * bCols) : undefined;
  ...
  if (isRuntimeTensor(a) && isRuntimeSparseMatrix(b)) {
    ...
    const result = allocFloat64Array(aRows * b.n);
    const resultImag = hasImag ? allocFloat64Array(aRows * b.n) : undefined;
...
export function mElemDivSparse(a: RuntimeValue, b: RuntimeValue): RuntimeValue {
  ...
  if (isRuntimeSparseMatrix(b)) {
    ...
    if (cs) {
      ...
      const data = allocFloat64Array(len);
      const imag = hasImag ? allocFloat64Array(len) : undefined;
      for (let i = 0; i < len; i++) {
  ...
  const data = allocFloat64Array(aDense.data.length);
  const imag = hasImag ? allocFloat64Array(aDense.data.length) : undefined;
