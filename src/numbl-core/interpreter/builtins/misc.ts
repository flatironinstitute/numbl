  ...
  registerIBuiltin({
    ...
    resolve: () => ({
      ...
      apply: rawArgs => {
        ...
        const t = RTV.tensor(
          fillVal
            ? allocFloat64Array(rows * cols).fill(1)
            : allocFloat64Array(rows * cols),
          [rows, cols]
        );
        t._isLogical = true;
        return t;
    ...
    apply: (args: RuntimeValue[]) => {
      ...
      if (args.length >= 2) {
        ...
        if (propStr.toLowerCase() === "colormap") {
          const n = 64;
          const data = allocFloat64Array(n * 3);
          for (let i = 0; i < n; i++) {
            const t = i / (n - 1);
...
// `set(handle, 'PropName', value, ...)` is registered as a runtime-closured
// special builtin in specialBuiltins.ts (it needs to push `update_trace`
// instructions so live edits reach the serialized plot viewer).
...
for (const name of ["xlim", "ylim"]) {
  registerIBuiltin({
    ...
    resolve: () => ({
      outputTypes: [{ kind: "tensor", isComplex: false }],
      apply: () => RTV.tensor(allocFloat64Array(0), [0, 0]),
    }),
  });
}
...
function convertJsonArray(arr: unknown[]): RuntimeValue {
  if (arr.length === 0) return RTV.tensor(allocFloat64Array(0), [0, 0]);
  ...
  if (allBooleans && !allNumbers) {
    ...
    const data = allocFloat64Array(arr.length);
