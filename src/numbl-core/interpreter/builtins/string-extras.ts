...
function regexpImpl(
  ...
  function buildOutput(mode: string): RuntimeValue {
    if (mode === "start") {
      if (matchOnce)
        return starts.length > 0
          ? RTV.num(starts[0])
          : RTV.tensor(allocFloat64Array(0), [1, 0]);
      return RTV.tensor(allocFloat64Array(starts), [1, starts.length]);
    }
    if (mode === "end") {
      if (matchOnce)
        return ends.length > 0
          ? RTV.num(ends[0])
          : RTV.tensor(allocFloat64Array(0), [1, 0]);
      return RTV.tensor(allocFloat64Array(ends), [1, ends.length]);
    }
    if (mode === "match") {
      if (matchOnce)
    ...
    if (mode === "names") {
      if (starts.length === 0) return RTV.tensor(allocFloat64Array(0), [0, 0]);
    ...
    // tokenextents
    if (matchOnce) return RTV.tensor(allocFloat64Array(0), [1, 0]);
