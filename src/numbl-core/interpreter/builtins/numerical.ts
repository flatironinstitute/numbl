...
function toFloatArray(v: RuntimeValue): FloatXArrayType {
  if (isRuntimeNumber(v)) return copyFloatX([v as number]);
  if (isRuntimeLogical(v)) return copyFloatX([v ? 1 : 0]);
  if (isRuntimeTensor(v)) return copyFloatX(v.data);
  ...
  cases: [
    {
      ...
      apply: args => {
        ...
        const fullLen = m + n - 1;
        const full = zeroedFloatX(fullLen);
        for (let i = 0; i < m; i++) {
          for (let j = 0; j < n; j++) {
            full[i + j] += a[i] * b[j];
          }
  ...
  cases: [
    {
      ...
      apply: (args, nargout) => {
        ...
        const nq = nb - na + 1;
        if (nq <= 0) {
          if (nargout <= 1) return RTV.tensor(copyFloatX([0]), mkShape(1));
          return [
            RTV.tensor(copyFloatX([0]), mkShape(1)),
            RTV.tensor(copyFloatX(b), mkShape(nb)),
          ];
        }

        const r = copyFloatX(b);
        const q = zeroedFloatX(nq);
        for (let i = 0; i < nq; i++) {
          q[i] = r[i] / a[0];
          for (let j = 0; j < na; j++) {
            r[i + j] -= q[i] * a[j];
  ...
  cases: [
    {
      ...
      apply: args => {
        ...
        if (isRuntimeTensor(x)) {
          const result = zeroedFloatX(x.data.length);
  ...
  cases: [
    {
      ...
      apply: args => {
        ...
        const result = zeroedFloatX(ncols);
  ...
  cases: [
    {
      ...
      apply: (args, nargout) => {
        ...
        const sp = hScalar ?? 1;
        const fxData = zeroedFloatX(nRows * nCols);
        for (let r = 0; r < nRows; r++) {
          const rowSlice = zeroedFloatX(nCols);
        ...
        if (nargout >= 2) {
          const fyData = zeroedFloatX(nRows * nCols);
          for (let c = 0; c < nCols; c++) {
            const colSlice = zeroedFloatX(nRows);
  ...
  cases: [
    {
      ...
      apply: args => {
        ...
        const result = zeroedFloatX(maxIdx);
  ...
  cases: [
    {
      ...
      apply: args => {
        ...
        if (isRuntimeTensor(xqArg)) {
          const result = zeroedFloatX(xqArg.data.length);
  ...
  cases: [
    {
      ...
      apply: args => {
        ...
        if (n <= 0) return RTV.tensor(zeroedFloatX(0), [0, 0]);

        const C = zeroedFloatX(n * n);
        for (let i = 0; i < n - 1; i++) C[(i + 1) * n + i] = 1;
        for (let i = 0; i < n; i++) C[i] = -p[i + 1] / p[0];

        const bridge = getEffectiveBridge("roots", "eig");
        if (bridge.eig) {
          const f64 = C instanceof Float64Array ? C : copyFloat64(C);
          ...
          if (eigResult) {
            ...
            if (hasComplex) {
              const realPart = zeroedFloatX(n);
              const imagPart = zeroedFloatX(n);
              for (let i = 0; i < n; i++) {
        ...
        if (n === 1) {
          return RTV.tensor(copyFloatX([-p[1] / p[0]]), [1, 1]);
        }
        if (n === 2) {
          const a = p[0],
            b = p[1],
  ...
  cases: [
    {
      ...
      apply: args => {
        ...
        if (isRuntimeNumber(A)) {
          return RTV.tensor(copyFloatX([1, -(A as number)]), [1, 2]);
        }
        ...
        if (m === 1 || n === 1) {
          ...
          let coeffs = zeroedFloatX(nr + 1);
          coeffs[0] = 1;
          for (let i = 0; i < nr; i++) {
            const newCoeffs = zeroedFloatX(nr + 1);
        ...
        if (bridge.eig) {
          const f64 =
            A.data instanceof Float64Array ? A.data : copyFloat64(A.data);
          const eigResult = bridge.eig(
            f64 as Float64Array,
            n,
            false,
          ...
          if (eigResult) {
            const roots = zeroedFloatX(n);
            for (let i = 0; i < n; i++) roots[i] = eigResult.wr[i];
            let coeffs = zeroedFloatX(n + 1);
            coeffs[0] = 1;
            for (let i = 0; i < n; i++) {
              const newCoeffs = zeroedFloatX(n + 1);
              newCoeffs[0] = coeffs[0];
              for (let j = 1; j <= i + 1; j++) {
                newCoeffs[j] = coeffs[j] - roots[i] * coeffs[j - 1];
              }
        ...
        if (n === 2) {
          ...
          return RTV.tensor(copyFloatX([1, -tr, dt]), [1, 3]);
        }
  ...
  cases: [
    {
      ...
      apply: args => {
        if (
          ...
          const d = n - 1;
          return RTV.tensor(
            copyFloatX([sxx / d, sxy / d, sxy / d, syy / d]),
            [2, 2]
          );
        }
        ...
        const means = zeroedFloatX(ncols);
        for (let j = 0; j < ncols; j++) {
          let s = 0;
        ...
        const result = zeroedFloatX(ncols * ncols);
  ...
  cases: [
    {
      ...
      apply: args => {
        ...
        const covMat = zeroedFloatX(ncols * ncols);
        for (let p = 0; p < ncols; p++) {
          for (let q = 0; q < ncols; q++) {
            let s = 0;
            for (let i = 0; i < m; i++) {
  ...
  cases: [
    {
      ...
      apply: (args, nargout) => {
        ...
        const counts = zeroedFloatX(countShape[0] * countShape[1]);
        const indData = zeroedFloatX(xRows * xCols);
  ...
  cases: [
    {
      ...
      apply: args => {
        ...
        if (isRuntimeNumber(coefsArg) || isRuntimeLogical(coefsArg)) {
          ...
          coefData = copyFloatX(coefsArg.data);
        } else {
        ...
        const breaksOut = zeroedFloatX(Lp1);
        ...
        if (dim.length === 1) {
          ...
          fields.set("dim", RTV.tensor(copyFloatX(dim), [1, dim.length]));
        }
  ...
  cases: [
    {
      ...
      apply: args => {
        ...
        if (isRuntimeNumber(xq) || isRuntimeLogical(xq)) {
          ...
          if (isScalarDim) {
            const out = zeroedFloatX(1);
            evalAt(xv, out, 0);
            return RTV.num(out[0]);
          }
          const out = zeroedFloatX(dProd);
          evalAt(xv, out, 0);
        ...
        const out = zeroedFloatX(dProd * N);
