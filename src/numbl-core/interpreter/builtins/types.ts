  ...
  if (isRuntimeTensor(v)) {
    ...
    if (!v.imag) {
      ...
      if (nativeOpCode !== undefined && v.data instanceof Float64Array) {
        const nativeOut = allocFloat64(n);
        ...
        const outR = allocFloatX(n);
        const outI = zeroedFloatX(n); // must zero-init: only NaN slots get im written
        let hasImag = false;
        for (let i = 0; i < n; i++) {
          if (!Number.isNaN(nativeOut[i])) {
            outR[i] = nativeOut[i];
...
export function applyBinaryElemwise(
  ...
  if (aIsTensor && bIsNum) {
    ...
    const out = allocFloatX(t.data.length);
  ...
  if (aIsTensor && bIsTensor) {
    ...
    const out = allocFloatX(tA.data.length);
