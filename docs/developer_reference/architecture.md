...
5. **JIT (optional)** — hot user functions, hot loops, and feasible top-level scripts are specialized. The JIT lowers AST to a typed IR, then emits JS (`--opt 1`, the default). Under `--opt e3`, named C-JIT optimizers compete with the JS-JIT executors via the executor registry. Runtime type mismatches bail back to the interpreter.
...
| JIT              | Type-specializes hot code to JS (and to C under `--opt e3`).                     |
