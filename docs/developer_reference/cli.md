...
- `--opt <0|1|2>` — interpreter / JS-JIT / C-JIT (see [jit/overview.md](jit/overview.md)).
- `--dump-js` and `--dump-c` — write JIT-generated source to disk for inspection.
...
C-JIT-specific environment variables (`NUMBL_CC`, `NUMBL_CFLAGS`, `NUMBL_NO_NATIVE_CFLAGS`) control how the C backend compiles its output.
