...
A recursive-descent parser, split into focused sub-parsers by category (expressions, statements, control flow, functions, classes, command form, argument blocks). Output is an `AbstractSyntaxTree` — an array of `Stmt` nodes.

### AST shape

- `Stmt` is the discriminated union of statements: assignment, expression-statement, if/for/while/switch/try, function/class declarations, arguments block, etc.
