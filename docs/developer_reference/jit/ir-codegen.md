...
Builtins emit through the JIT compiler's own registry (`jit/builtins`): each def supplies `emitJs` / `emitC`. **Runtime snippets** (`jit/builtins/runtime/*.h` C bodies + `*.js` siblings) are generated by `scripts/build_runtime_snippets.ts` into two files: the JS bodies + dep graph into `snippets.gen.ts`, and the C bodies (`C_SNIPPETS`) into `snippets.c.gen.ts`. They are inlined into the emitted module **lazily** — only snippets activated by the emitted code are included. The C table is split out so the browser worker bundle excludes its ~300 KB of C source: C-JIT is Node-only, so the Node bootstrap (`executors/jit/compileC.node.ts → registerNodeCompileC`) registers `C_SNIPPETS` with the codegen runtime via `setCSnippets`; `codegen/runtime.ts` resolves C bodies lazily and never references the C table in the browser-reachable graph.
