...
| Owned binop operands not disposed after consumption                                                      | chained binops (`a*2 + 1`) leak the intermediate                                                                                                             | binop dispatch could dispose owned operand tensors after producing the result                                                                                                                                                                                      |
...
| `for` loop iteration with rebinding leaks one per iteration                                              | `total = total + 1` in a loop body — Assign disposes the _previous_ binding, but the binop's owned operand (the rhs `total + 1` source path) is not disposed | needs the binop-owned-operand fix above                                                                                                                                                                                                                            |
