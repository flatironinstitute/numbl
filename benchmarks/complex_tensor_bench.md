...
npx tsx src/cli.ts run benchmarks/complex_tensor_bench.m --opt e1   # JS-JIT + inline C kernels
...
| Kernel                    | `--opt 1` | `--opt e1` | MATLAB (1 thread) | MATLAB (8 threads) |
| ------------------------- | --------: | ---------: | ----------------: | -----------------: |
| 1. Mandelbrot z.\*z+c     |     0.146 |      0.054 |             0.276 |              0.061 |
| 2. Tensor chain z.\*w+y   |     0.197 |      0.098 |             0.278 |              0.071 |
| 3. Conj chain conj(z).\*z |     0.468 |      0.060 |             0.359 |              0.130 |
| 4. Widening x+y\*1i       |     0.146 |      0.047 |             0.249 |              0.059 |
| 5. Divide z./w            |     0.117 |      0.081 |             0.265 |              0.073 |
| 6. abs + sum reduction    |     0.105 |      0.078 |             0.267 |              0.075 |
| **Total**                 |     1.228 |      0.420 |             1.702 |              0.470 |

`--opt e1` emits a paired-buffer C kernel for complex chains (real/imag
pointer pairs per tensor, `re`/`im` arg pairs per complex scalar) —
see [src/numbl-core/jit/e1/complexKernelEmit.ts](../src/numbl-core/jit/e1/complexKernelEmit.ts).
Kernel 3 (`conj(z).*z`) is a 7.8× win over `--opt 1` because the per-op
complex path walks the chain three times through libnumbl_ops (one call
per op); e1 collapses the three ops into a single 6-flop per-element
body. Kernels 5–6 still run per-op since `./` (Smith's method branches
break SIMD) and `abs(complex)` (complex→real type transition mid-chain)
are outside the fused-emitter envelope.

`--opt e1 --par` deliberately does NOT thread complex chains: the
...
- Supports complex tensor `+ - .* ./`, unary minus, `conj`, `real`,
  `imag`, `abs`, and flat reductions (`sum`, `prod`, `any`, `all`).
  Anything more (transcendentals, index read/write, range slices) still
  bails to JS-JIT.
- `-ffast-math` is enabled for the C kernel compile; reduction results
